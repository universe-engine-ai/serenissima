/**
 * Script to run the relationship evaluation between SilentObserver and RialtoRacer56
 */

const fs = require('fs');
const path = require('path');

// Load the system context data
const systemContextPath = path.join(__dirname, 'addSystem.txt');
let systemContextData;

try {
  const fileContent = fs.readFileSync(systemContextPath, 'utf8');
  
  // Extract the JSON data from the text file
  // The file contains markdown-formatted content with a JSON object
  const jsonMatch = fileContent.match(/```([\s\S]*?)```/);
  
  if (jsonMatch && jsonMatch[1]) {
    // Parse the JSON data
    systemContextData = JSON.parse(jsonMatch[1]);
  } else {
    throw new Error('Could not extract JSON data from system context file');
  }
} catch (error) {
  console.error('Error loading system context data:', error.message);
  process.exit(1);
}
...
function evaluateRelationship(data) {
  ...
  // Determine relationship title and description based on scores and relevancies
  let title = "";
  let description = "";
...
// Run the evaluation with the loaded data
const result = evaluateRelationship(systemContextData);
