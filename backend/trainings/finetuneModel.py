...
3. Fine-tunes a pre-trained model using standard techniques (no LoRA/PEFT)
...
# Constantes pour le fine-tuning
...
# Fonction pour déterminer les paramètres optimaux en fonction du modèle
def get_training_parameters(model_name):
    """
    Déterminer les paramètres de training optimaux en fonction du modèle.
    """
    params = {
    ...
    if "qwen" in model_name.lower() or "deepseek" in model_name.lower():
        params["learning_rate"] = 1e-5  # Taux d'apprentissage plus faible pour les grands modèles
...
def main():
    ...
    # Obtenir les paramètres de training optimaux pour ce modèle
    training_params = get_training_parameters(model_name)
    
    # Utiliser les paramètres de ligne de commande s'ils sont fournis
    ...
    log.info(f"Paramètres d'entraînement: {training_params}")
    ...
    try:
        ...
        log.info("Préparation du modèle pour l'entraînement standard (sans LoRA)")
