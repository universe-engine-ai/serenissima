...
-   `Type` (Texte): Type d'activité ou d'action (ex: `rest`, `goto_home`, `production`, `bid_on_land` (peut être conservé pour des enchères plus complexes), `send_message`, `manage_public_sell_contract`, `manage_public_dock`, `list_land_for_sale`, `make_offer_for_land`, `accept_land_offer`, `buy_listed_land`, `cancel_land_listing`, `cancel_land_offer`, `buy_available_land`, `inspect_building_for_purchase`, `submit_building_purchase_offer`, `work_on_art`, `read_book`, `deposit_items_at_location`, `attend_theater_performance`, `drink_at_inn`).
