...
def try_create(tables: dict, citizen_record: dict, activity_type: str, activity_parameters: dict,
    ...
    """
    Creates activities for a citizen to cancel their land listing.
    """
    activities_created = []
    citizen_username = citizen_record['fields'].get('Username') # This is the seller cancelling
    ...
    if not listing_contract_id or not land_id:
        log.error(f"{LogColors.FAIL}Missing contractId or landId for cancel_land_listing for citizen {citizen_username}. Params: {activity_parameters}{LogColors.ENDC}")
        return []

    # Verify the listing contract exists, is active, and belongs to the citizen
    listing_contract_record = get_contract_record(tables, listing_contract_id)
    if not listing_contract_record:
        log.error(f"{LogColors.FAIL}Listing contract {listing_contract_id} not found. Cannot cancel. Activity for {citizen_username}.{LogColors.ENDC}")
        return []
    
    listing_fields = listing_contract_record['fields']
    if listing_fields.get('Type') != 'land_listing' or listing_fields.get('Status') != 'active':
    ...
    log.info(f"{LogColors.ACTIVITY}Attempting to create 'cancel_land_listing' activity chain for {citizen_username} for listing {listing_contract_id} on land {land_id}.{LogColors.ENDC}")
    ...
    preferred_office_types = ["town_hall", "public_archives", "notary_office"] # Or citizen's home/business
    ...
    if not target_office_record:
        for office_type in preferred_office_types:
            ...
            if found_office:
                ...
                break
        if not target_office_record:
            log.error(f"{LogColors.FAIL}No suitable office found for {citizen_username} to cancel listing.{LogColors.ENDC}")
            return []
    ...
    if path_data and path_data.get("path"):
        ...
        goto_activity = {
            ...
            "Description": f"{citizen_username} is traveling to {target_office_record['fields'].get('Name', target_office_building_id)} to cancel land listing {listing_contract_id}.",
            "Thought": f"I need to go to the {target_office_record['fields'].get('Type', 'office')} to cancel my listing for land {land_id}.",
    ...
    # 4. Create execute_cancel_land_listing activity
    ...
    execute_activity_details = json.dumps({
        "listingContractId": listing_contract_id,
        "landId": land_id,
        "cancellerUsername": citizen_username 
    })

    execute_activity = {
        "ActivityId": execute_activity_id, "Citizen": citizen_username, "Type": "execute_cancel_land_listing", "Status": "created", # Use username
        ...
        "Title": f"Cancel Land Listing {listing_contract_id}",
        "Description": f"{citizen_username} is finalizing the cancellation of listing {listing_contract_id} for land {land_id} at {target_office_record['fields'].get('Name', target_office_building_id)}.",
        "Thought": f"I've decided to take my land {land_id} off the market for now.",
        "CreatedAt": now_utc_dt.isoformat(), "UpdatedAt": now_utc_dt.isoformat()
    }
    activities_created.append(execute_activity)
    log.info(f"{LogColors.ACTIVITY}Created execute_cancel_land_listing activity {execute_activity_id} for {citizen_username}. Starts at {execute_start_time_utc.isoformat()}.{LogColors.ENDC}")
