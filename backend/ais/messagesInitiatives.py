...
def generate_ai_initiative_message(tables: Dict[str, Table], ai_username: str, target_username: str) -> Optional[str]:
    ...
    try:
        ...
        kinos_prompt = (
            f"You are {ai_display_name}, a citizen of Venice. Start a very short, casual, and human-like conversation with {target_display_name}.\n"
            f"Focus on a natural, direct opening. Do NOT mention sending a message or initiating contact. Avoid formal language, long paragraphs, and fluff.\n\n"
            f"Make your message RELEVANT to {target_display_name} and FOCUSED ON GAMEPLAY, using the provided 'addSystem' context:\n"
            f"- Your profile ('ai_citizen_profile')\n"
            f"- {target_display_name}'s profile ('target_citizen_profile')\n"
            f"- Your relationship ('relationship_with_target')\n"
            f"- Recent news/events ('recent_notifications_for_ai')\n"
            f"- Why {target_display_name} is relevant to you ('recent_relevancies_ai_to_target')\n"
            f"- Problems involving either of you ('recent_problems_involving_ai_or_target')\n"
            f"- Consider discussing a relevant stratagem (e.g., 'Coordinate Pricing', 'Undercut') if it aligns with recent issues or opportunities.\n\n"
            f"Your message (start naturally, be concise):"
        )
